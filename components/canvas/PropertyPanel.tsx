...
const fields: Record<string, Field[]> = {
  ...
  image: [{ key: "title", label: "Title" }, { key: "prompt", label: "Image prompt", kind: "textarea" }, { key: "model", label: "Model override (blank = server default)" }, { key: "size", label: "Size", kind: "select", options: ["1024x1024", "1536x1024", "1024x1536", "auto"] }],
  video: [{ key: "title", label: "Title" }, { key: "videoProvider", label: "Video provider", kind: "select", options: ["", "mock", "302ai", "302-sora2", "tokenstar", "kling"] }, { key: "prompt", label: "Motion prompt", kind: "textarea" }, { key: "referenceImageUrl", label: "First frame URL (Kling, optional)" }, { key: "model", label: "Model override (not used by 302-sora2 or Kling)" }, { key: "tokenstarMode", label: "TokenStar mode (asset-video uses connected media)", kind: "select", options: ["text-to-video", "asset-video"] }, { key: "referenceImageAssetUrl", label: "Existing image asset URL (asset://…, optional)", kind: "textarea" }, { key: "referenceVideoAssetUrl", label: "Existing video asset URL (asset://…, optional)", kind: "textarea" }, { key: "referenceAudioAssetUrl", label: "Existing audio asset URL (asset://…, optional)", kind: "textarea" }, { key: "videoInputMode", label: "302 generation mode", kind: "select", options: ["text-to-video", "image-to-video"] }, { key: "duration", label: "Duration (Kling: 3-15; Sora: 4, 8, or 12)", kind: "number" }, { key: "resolution", label: "Resolution (720p or 1080p)" }, { key: "fps", label: "FPS" }, { key: "aspectRatio", label: "Aspect ratio", kind: "select", options: ["16:9", "9:16", "1:1"] }, { key: "generateAudio", label: "Generate audio", kind: "select", options: ["true", "false"] }],
...
export function PropertyPanel() {
  ...
  if (!node) return (
    <aside className="w-72 shrink-0 border-l border-[#e7eaf0] bg-white p-4 dark:border-slate-800 dark:bg-[#0c1622]">
      <p className="text-xs font-semibold uppercase tracking-[.16em] text-[#939393] dark:text-slate-500">Inspector</p>
      <p className="mt-5 text-sm leading-6 text-[#676f7b] dark:text-slate-500">
        Select a node to edit its settings and review generated output.
      </p>
    </aside>
  );
  ...
  return (
    <aside className="w-72 shrink-0 overflow-y-auto border-l border-[#e7eaf0] bg-white p-4 dark:border-slate-800 dark:bg-[#0c1622]">
          ...
          <label className="block" key={field.key}>
            ...
            {field.kind === "textarea" ? (
              ...
              <Select value={String(node.data[field.key] ?? "")} onChange={(event) => change(field.key, event.target.value)}>
                {field.options?.map((option) => <option key={option}>{option || "Server default"}</option>)}
              </Select>
            ) : (
      ...
      {node.data.nodeType === "storyboardImage" && (
        ...
        >
          Generate {prompts.length || ""} keyframes
        </button>
      )}
      {node.data.output && (
        <div className="mt-6 border-t border-[#e7eaf0] pt-4 dark:border-slate-800">
          <p className="text-xs font-semibold uppercase tracking-[.16em] text-[#939393] dark:text-slate-500">Last output</p>
